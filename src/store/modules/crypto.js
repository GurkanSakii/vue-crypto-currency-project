...
const getters = {
  ...
  cryptoPortfolioFullData: (state) => {
    return state.cryptoData.filter((crpyto) => state.portfolio.includes(crpyto.id));
  },
};

const actions = {
  setShowLoad: ({ commit }, value) => {
  ...
  fetchExchanges: async ({ commit }) => {
    ...
    const {
      data: { data },
    } = await cryptoApi.fetchData(CRYPTO_API.BASE_URL, CRYPTO_API.EXCHANGE_PATH);
  ...
  removeFromPortfolio: ({ commit }, targetCryptoId) => {
    ...
    const updatedPortFolio = portfolio.filter((crypto) => crypto !== targetCryptoId);

    window.localStorage.setItem(CRYPTO_PORTFOLIO, JSON.stringify(updatedPortFolio));
